...
def polarToXY(x,y,t,r):
    T = 2*pi/3
    tOffset = pi/3
    t1 = math.floor((t+tOffset)/T) * T
    t2 = t1 + T
    x1,y1 = x+r*cos(t1),y+r*sin(t1)
    x2,y2 = x+r*cos(t2),y+r*sin(t2)
    w2 = (t - t1) / T
    w1 = 1.0 - w2
    return x1*w1 + x2*w2, y1*w1 + y2*w2
...
##     return [
##         (piddle.figureLine, x0-r0, y0-r0, x0+r0, y0+r0),
##         (piddle.figureLine, x1-r1, y1-r1, x1+r1, y1+r1),
##         (piddle.figureArc, x0-r0, y0-r0, x0+r0, y0+r0, 0, 180),
##         (piddle.figureArc, x1-r1, y1-r1, x1+r1, y1+r1, 0, 180),
##        ]
    return [
