...
;; I usually put a single space between sentences.
...
(defun maybe-kill-emacs ()
  "Ask the user if she wants to exit emacs."
  (interactive)
  (if (yes-or-no-p "Exit Emacs? ")
...
;; (defun vague-view ()
;;   "Run vague and view resulting PS file."
;;   (interactive)
;;   (save-buffer)
  
;;   (shell-command (file-truename)
;;   (setq font-setup-font-number (1+ font-setup-font-number))
;;   (if (>= font-setup-font-number (length font-setup-font-list))
;;       (setq font-setup-font-number 0))
;;   (let ((fonts (aref font-setup-font-list font-setup-font-number)))
...
;;(toggle-auto-compression +1)  ; definitely not GNU Emacs
...
(defun my-c-mode-common-hook ()
  ...
  (define-key c-mode-base-map "\M-\C-m" 'indent-new-comment-line)
...
;;  (set-face-property 'modeline 'background "gray80")
...
(define-key global-map [f3] 'insert-total-time)
...
(define-key global-map "\C-co" 'find-files)
...
(define-key global-map "\C-cx" 'font-setup-switch)
...
;; All this works fine in GNU Emacs automatically.
;; Hack it in XEmacs.
(cond 
 ((and (featurep 'mule) running-xemacs)
...
(setq smtpmail-smtp-server "smtp.west.cox.net")
(setq smtpmail-local-domain "west.cox.net")
(setq smtpmail-sendto-domain "cox.net")
(setq smtpmail-debug-info t) ; only to debug problems

(setq smtpmail-auth-credentials
      '(("smtp.west.cox.net" 25 "joel.sullivan" nil)))
